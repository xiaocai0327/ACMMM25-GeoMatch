...
from geomatch.dataset.vigor import VigorDatasetEval, VigorDatasetTrainGeomatch,VigorDatasetTrainGeomatch_All
...
from geomatch.trainer import train_contrast_geomatch
from geomatch.evaluate.vigor import evaluate, calc_sim
from geomatch.evaluate.university import evaluate as evaluate_university
from geomatch.loss import InfoNCE
from geomatch.model import TimmModel_Geomatch
...
if __name__ == '__main__':
    ...
    model = TimmModel_Geomatch(config.model,
                          pretrained=False,
    ...
    # Transforms
    sat_transforms_train1, sat_transforms_train2, ground_transforms_train1, ground_transforms_train2 = get_transforms_train_geomatch(image_size_sat,
                                                                   img_size_ground,
                                                                   mean=mean,
                                                                   std=std,
                                                                   fov=train_fov
    ...
    # Train
    train_dataset = VigorDatasetTrainGeomatch_All(data_folder=config.data_folder,
    ...
    val_transforms,_ , _ , _ , _ = get_transforms_train_geomatch_u((img_size,img_size),(img_size,img_size) ,mean=mean, std=std)
    ...
    for epoch in range(start_epoch+1, config.epochs+1):
        ...
        train_loss =  train_contrast_geomatch(config,
                           model,
